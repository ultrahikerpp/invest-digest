...
# 抓取單一標的財報資料並產生 Claude 分析
./venv/bin/python runner.py earnings <ticker>

# 智慧刷新 earnings_watchlist.json 中所有標的的財報
# （已是最新就跳過、同季僅更新數字、新季度才完整重新抓取 + 重新分析）
./venv/bin/python runner.py refresh-earnings
./venv/bin/python runner.py refresh-earnings --deploy   # 刷新後自動 build + 部署
./venv/bin/python runner.py refresh-earnings --force    # 強制對所有標的完整重新抓取
```
